...
- [x] B5.2 Wall-clock session ceiling (deps: B5.1, est: ~40m)
  ...
  - landed: core/vite/routes/review.ts (`DEFAULT_LIFETIME_MS`, `ReviewDeps.lifetimeMs`, `lifetimeTimer` + `onExpired`), core/vite/routes/review.test.ts (5 guards), ui/review-view.tsx (`ENDINGS.expired`). **One hour, not fifteen minutes.** The idle bound is apply's ceiling reused because both reap something nobody is attending; this one bounds a conversation a human is *in*, so tuning it to apply's number would reap working sessions — a bug wearing a requirement's clothes. Four idle bounds is longer than any refine-and-approve run the spikes produced and short enough that a tab left open over lunch returns the shared `RunLock` the same afternoon. **The timer is armed in `start()` and read nowhere else.** `armIdle()` is called by `subscribe`, the subscriber's `close`, `message`, and each turn end; the ceiling deliberately appears in none of them, because a ceiling any activity can push back is not a ceiling — and activity is exactly what defeats R-7.6, whose bound is a conjunction (no client **and** no input) that a silent user with an open tab never satisfies. **`'expired'` is a fifth `ReviewEndReason`, not a second use of `'idle'`:** the two say opposite things about the user — left alone versus used too long — so conflating them tells a user who was mid-conversation something untrue, and `'error'` tells them something worse, since nothing failed. **The test that matters is the one B5.1 could not write:** a subscriber attached for the whole session *and* three delivered turns, so `timers.armed()` is 0 throughout and the idle reaper provably cannot be what ends it. The fake scheduler now tells the two timers apart by their bound rather than by insertion order, and `total()` proves both are cleared on every terminal path.
