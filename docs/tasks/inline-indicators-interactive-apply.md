...
- [ ] B4.1 Approve → apply the approved patch (deps: B3.1, B1.1, est: ~2h)
...
- [ ] B4.2 Drift gate — patch refuses to apply (deps: B4.1, est: ~40m)
...
- [ ] B4.3 Fallback labelling — scoped apply pass (deps: B4.1, est: ~40m)
...
- [ ] B6.1 Per-comment review entry point (deps: B2.2, est: ~1h)
...
- [ ] B6.2 Review view — proposal, patch, follow-up input, approve/cancel (deps: B3.1, B4.2, B5.1, B6.1, est: ~3h)
