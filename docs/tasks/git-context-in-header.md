...
- [ ] 1.1 Write `core/git-context.ts` over `git`, never throwing (est: ~45m)
...
- [ ] 1.2 Parse the remote URL, and keep the raw one when it will not parse (deps: 1.1, est: ~30m)
...
- [ ] 1.3 Keep the git reader out of the browser bundle (deps: 1.1, est: ~20m)
...
- [ ] 2.1 Add `core/vite/routes/git.ts` and wire both hosts (deps: 1.2, est: ~30m)
...
- [ ] 2.2 Read per request, with no cache (deps: 2.1, est: ~10m)
...
- [ ] 2.3 Put the route behind the guard, and 404 when absent (deps: 2.1, est: ~15m)
...
- [ ] 2.4 Record the served-directory boundary crossing as an ADR (deps: 2.1, est: ~20m)
...
- [ ] 3.1 Write `ui/use-git-context.ts` — read on mount, focus and visibility (deps: 2.1, est: ~30m)
...
- [ ] 3.2 Render the chip's states, including before the first read (deps: 3.1, est: ~40m)
...
- [ ] 3.3 Link only GitHub, and mark a detached HEAD as one (deps: 3.2, est: ~20m)
...
- [ ] 4.1 Show the active branch in the apply scope chooser (deps: 3.1, est: ~25m)
