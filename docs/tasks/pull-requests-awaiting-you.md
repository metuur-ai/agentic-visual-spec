...
- [x] 2.1 Add `searchPullRequests(repo, qualifier, login)` to the GitHub adapter (est: ~40m)
  ...
  - landed: 36bcfb3 — core/collaboration/github-adapter.ts, github-adapter.test.ts

- [x] 2.2 Resolve and validate the login server-side (deps: 2.1, est: ~25m)
  ...
  - landed: 36bcfb3 — core/vite/routes/collab.ts, collab.pulls.test.ts

- [x] 2.3 Build the mentions union from search plus the open pull requests' review comments (deps: 2.1, est: ~60m)
...
- [x] 2.4 Read review comments only for listed open pull requests, and only those that moved (deps: 2.3, est: ~35m)
...
- [x] 2.5 Match a mention without over- or under-matching (deps: 2.3, est: ~20m)
...
- [x] 2.6 Scope both counts to open pull requests of the configured repository (deps: 2.1, 2.3, est: ~15m)
...
- [x] 2.7 Expose `GET /__vs/collab/pulls/awaiting`, above the document-scoped match (deps: 2.2, 2.6, est: ~40m)
...
- [x] 2.8 Guard the inbox prohibition in the test suite (deps: 2.7, est: ~15m)
  ...
  - landed: 36bcfb3 — core/vite/routes/collab.pulls.test.ts
...
- [x] 4.1 Shape the payload so each side can fail alone (deps: 2.7, est: ~30m)
...
- [x] 4.2 Read on mount, focus and visibility — once per tab switch (deps: 4.1, est: ~30m)
  ...
  - landed: 36bcfb3 — ui/use-awaiting-pulls.ts, use-awaiting-pulls.test.tsx, ui/use-collab-pulls.ts, use-collab-pulls.test.tsx, ui/collab-client.ts

- [x] 4.3 Retain the last known counts through a failure (deps: 4.2, est: ~20m)
  ...
  - landed: 36bcfb3 — ui/use-awaiting-pulls.ts, use-awaiting-pulls.test.tsx

- [x] 4.4 Keep the header and listing alive when these reads fail (deps: 4.3, est: ~15m)
  ...
  - landed: 36bcfb3 — ui/use-awaiting-pulls.ts, ui/pull-count-chip.test.tsx

## Unit 1: The chips

- [x] 1.1 Render the two counts as two chips (deps: 4.2, est: ~40m)
  ...
  - landed: 36bcfb3 — ui/main-header.tsx, ui/App.tsx, ui/git-chip.test.tsx

- [x] 1.2 Show nothing for a count that is zero or not yet known (deps: 1.1, est: ~20m)
  ...
  - landed: 36bcfb3 — ui/main-header.tsx, ui/git-chip.test.tsx

- [x] 1.3 Count a pull request in both chips when it qualifies for both (deps: 1.1, est: ~10m)
...
- [x] 1.4 Name the repository once for the group of counts (deps: 1.1, est: ~30m)
...
- [x] 3.1 Add the two sections to the existing pull request panel (deps: 1.1, est: ~45m)
  ...
  - landed: 36bcfb3 — ui/collab-pulls-panel.tsx, collab-pulls-panel.test.tsx

- [x] 3.2 Give matched rows the actions the panel already offers (deps: 3.1, est: ~20m)
...
- [x] 3.3 Render an unmatched pull request without offering a checkout (deps: 3.1, est: ~30m)
...
- [x] 3.4 Show who wrote the mention and what it said (deps: 3.1, est: ~30m)
...
- [x] 3.5 Say when a section shows fewer rows than it counts (deps: 3.3, est: ~20m)
...
- [x] 3.6 Keep the sections open-only when the listing is switched (deps: 3.1, est: ~20m)
