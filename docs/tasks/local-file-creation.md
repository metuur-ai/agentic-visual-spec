...
- [ ] 4.1 Answer 404 JSON for unmatched `/__vs/*` on both hosts (est: ~20m)
...
- [ ] 4.2 Parse a JSON body for non-GET methods on `/__vs/tree` in the standalone host (est: ~15m)
...
- [ ] 4.3 Freeze `core/vite/routes/` as host-agnostic with an import-graph test (est: ~20m)
...
- [ ] 1.1 Add `resolveForWrite` to `TreeStore` — string guard plus symlink guard (deps: none, est: ~40m)
...
- [ ] 1.2 Create `core/vite/routes/files.ts` with `POST /__vs/tree/create` validation (deps: 1.1, est: ~35m)
...
- [ ] 1.3 Complete create: collision, atomic write, seed (deps: 1.2, est: ~35m)
...
- [ ] 1.4 Wire the create route into both hosts (deps: 1.3, 4.1, 4.2, est: ~20m)
...
- [ ] 3.1 Add optional `invalidate()` to `TreeStore` and call it after a successful write (deps: 1.3, est: ~30m)
...
- [ ] 3.2 Confirm rename freshness through the same invalidation (deps: 3.1, 2.2, est: ~10m)
...
- [ ] 2.1 Add `POST /__vs/tree/rename` validation (deps: 1.2, est: ~30m)
...
- [ ] 2.2 Move the file non-destructively (deps: 2.1, est: ~25m)
...
- [ ] 2.3 Carry the review across the rename (deps: 2.2, est: ~35m, mutex: comment-doc)
...
- [ ] 2.4 Assert no delete operation is reachable (deps: 2.2, est: ~10m)
...
- [ ] 4.4 Assert host parity for both write routes (deps: 1.4, 2.2, est: ~25m)
...
- [ ] 4.5 Put both write routes behind the cross-origin guard and its attestation (deps: 1.4, 2.2, est: ~30m)
...
- [ ] 5.1 Add the "New file" control and its inline input (deps: 1.4, est: ~35m)
...
- [ ] 5.2 Close the loop on a successful create (deps: 5.1, 3.1, est: ~25m)
...
- [ ] 5.3 Add the per-row rename control and close its loop (deps: 5.1, 2.2, est: ~35m)
...
- [ ] 5.4 Report failures in the server's own words, and block double submits (deps: 5.1, 5.3, est: ~25m)
...
- [ ] 5.5 Confirm a created file is a first-class citizen (deps: 5.2, est: ~20m)
