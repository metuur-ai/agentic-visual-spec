 ...
 *
 * `.git` is listed twice on purpose. The trailing-slash form matches the directory of
 * an ordinary repository; a **linked worktree** (`git worktree add`) instead carries a
 * `.git` *file* holding `gitdir: …`, which the directory pattern does not match. Pull
 * request checkouts are linked worktrees, so without the bare entry the tree showed a
 * browsable `.git` at the root of every one of them. Found in the browser, not in a test.
 */
const ALWAYS_IGNORE = ['.git/', '.git', 'node_modules/', 'visual-spec-comments.json'];
